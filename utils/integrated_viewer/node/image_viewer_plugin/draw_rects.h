...
namespace integrated_viewer
{
    ...
    public:
        ...
        void DrawImageRect(const smartcar_msgs::DetectedObjectArray::ConstPtr &in_objects, cv::Mat &image);
        void DrawImageBox(const smartcar_msgs::DetectedObjectArray::ConstPtr &in_objects, cv::Mat &image);

    protected:
        static const int kRectangleThickness;

    private:
        void DrawLabel(const smartcar_msgs::DetectedObject& in_object, cv::Mat &image);
